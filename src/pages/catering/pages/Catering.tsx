...
const Catering = () => {
  ...
  useEffect(() => {
    ...
    if (paymentSuccess) {
      const finalizeOrder = async () => {
        try {
          const pendingOrderStr = localStorage.getItem("pendingCateringOrder");
          ...
          console.log("🚀 Finalizing Catering Order after payment...");
          ...
          if (response.ok) {
            ...
            console.log("✅ Catering Order Confirmed:", data);

            // Cleanup
            localStorage.removeItem("pendingCateringOrder");
            ...
            navigate("/catering/confirmation", {
              ...
              state: {
                ...
                orderDetails: data,
                extraDetails: orderData.extraDetails,
              },
            });
          } else {
            console.error("❌ Failed to finalize catering order");
            alert(
  ...
  useGoogleOneTapLogin({
    onSuccess: async (credentialResponse) => {
      try {
        ...
        // We don't need to reload, just let the state stay as is.
        // The components using token will react to it if they check it periodically, 
        // or we can force a small state change if needed.
      } catch (err) {
        console.error("Google One Tap Error:", err);
        toast.error("Google login failed.");
      }
    },
    onError: () => console.log("Google One Tap Skip/Error"),
  });

  // --- State Persistence & Loading ---
  useEffect(() => {
    const saved = localStorage.getItem("guestCateringOrder");
    if (saved) {
      try {
        const data = JSON.parse(saved);
        if (data.selectedEvent) setSelectedEvent(data.selectedEvent);
        if (data.selectedDetailedEventName) setSelectedDetailedEventName(data.selectedDetailedEventName);
        if (data.guestCount) setGuestCount(data.guestCount);
        if (data.selectedDateTime) setSelectedDateTime(data.selectedDateTime);
        if (data.step) setStep(data.step);
        if (data.selectedProvider) setSelectedProvider(data.selectedProvider);
        if (data.selectedServiceStyles) setSelectedServiceStyles(data.selectedServiceStyles);
        if (data.selectedMenuDescription) setSelectedMenuDescription(data.selectedMenuDescription);
        if (data.selectedCuisines) setSelectedCuisines(data.selectedCuisines);
        if (data.selectedCourses) setSelectedCourses(data.selectedCourses);
        ...
        if (data.selectedPax) setSelectedPax(data.selectedPax);
        if (data.selectedMenuItems) setSelectedMenuItems(data.selectedMenuItems);
      } catch (e) {
        console.error("Failed to load saved catering order", e);
      }
    }
  }, []);
  ...
  useEffect(() => {
    ...
    localStorage.setItem("guestCateringOrder", JSON.stringify(orderData));
  }, [
    selectedEvent,
    selectedDetailedEventName,
    guestCount,
  ...
  const handleGoBack = () => {
    ...
    if (step === 2)
      ...
      setStep(5); // Coffee Break Menu -> Budget
    else if (step === 10)
      setStep(5); // Platter Menu -> Budget
    else if (step === 12)
      setStep(5); // Live Station Menu -> Budget
    else if (step === 13)
      ...
      <main className="flex-1 bg-background">
        ...
        <LazyLoad>
          <div className="main-container !py-6 !pb-24">
            ...
            {/* Step 15: Ramadan Menu Selection */}
            {step === 15 && (
              <LazyLoad>
                <RamadanMenuSelection
                  selectedServiceStyles={selectedServiceStyles}
                  selectedBudget={selectedBudget}
                  handleGoBack={handleGoBack}
                  handleContinue={handleContinue}
