...
const MenuItemCard = ({
 ...
 return (
  <div
   ...
   className={`relative w-full border border-[#EDEEF2] max-w-[354px] bg-neutral-white rounded-[16px] px-3 pt-3 pb-5 sm:px-4 sm:pt-4 sm:pb-6 overflow-hidden cursor-pointer hover:shadow-lg transition-shadow ${isSoldOut || isLocked ? "opacity-60 grayscale-[0.5]" : ""}`}>
   <ImageWithShimmer
    src={data.image_url}
...
const VendingMenu = () => {
 ...
 const { currentDayItems, shelfData, totalAvailableCount } = useMemo(() => {
  ...
  if (selectedDayName === today && machineShelves) {
   ...
   const processedShelves = machineShelves
    .map((shelf) => ({
     ...
     spots: shelf.spots
      ...
      .filter(
       (spot: any) => spot.enrichedItem !== null, // Show Even if 0 stock (Sold Out), but MUST be mapped
      ),
    }))
   ...
   return {
    currentDayItems: [], // If we have machine shelves, we ONLY show those
   ...
   if (shelfData && shelfData.length > 0) {
    ...
    machineGoods.forEach((good: any) => {
     ...
     if (normalizeName(rawName) === normalizedItemName) {
      // FIX: Only add to totalAvailable if the good is NOT locked
      if (!good?.locked) {
    ...
    totalAvailable = 0; // No default ghost stock
   }
