...
const OrderNow = () => {
  ...
  const [machineGoods, setMachineGoods] = useState<any[] | null>(null); // Init as null to indicate loading
  ...
  // --- NEW: Fetch Machine Goods and Filter Menus with Caching ---
  useEffect(() => {
      ...
      try {
        ...
        if (cachedData) {
          ...
          if (goods && goods.length > 0) {
            console.log("🚀 Loading Machine Goods from Cache...");
            setMachineGoods(goods);
            if (!isExpired) return; // Cache is still fresh, no need to fetch
            console.log("♻️ Cache expired, fetching fresh data in background...");
          }
        }

        // 2. Fetch Machine Goods from Backend Proxy
        console.log("📦 Fetching Machine Goods from Backend Proxy...");
        ...
        if (data.data) {
          ...
          console.log("📦 Fresh Machine Goods Loaded:", allGoods.length, "items");
  ...
  // Helper to filter and enrich menu items
  const filterAndEnrichMenu = (menuItems: any[]) => {
    if (!menuItems) return [];
    if (machineGoods === null) return [];

    const normalizedMachineGoods = machineGoods.map(good => ({
      ...good,
      normalizedName: normalizeName(good.goodsName)
    }));

    const matchedItems = menuItems.filter(item => {
      const normalizedItemHeading = normalizeName(item.heading);
      const normalizedItemName = normalizeName(item.name);

      const match = normalizedMachineGoods.find(
        (good) => good.normalizedName === normalizedItemHeading || good.normalizedName === normalizedItemName
      );

      if (match) {
        item.machineUuid = match.uuid;
        item.machinePrice = match.goodsPrice;
        item.machineImage = match.goodsUrl;
        item.machineStock = true;
        return true;
      }
      return false;
    });

    const uniqueMatchedItems = Array.from(new Map(matchedItems.map(item => [item.heading, item])).values());
  ...
  useEffect(() => {
    if (machineGoods !== null) {
      // Re-filtering logic if needed
    }
  }, [machineGoods, originalOrderNowMenu]);

  const smartGrabAvailableItems = React.useMemo(() => {
    ...
    return machineGoods.map((good: any) => ({
      ...
      imgSrc: good.goodsUrl ? `http://pic.hnzczy.cn/${good.goodsUrl}` : "/images/placeholder-food.png",
      imgAlt: good.goodsName,
      description: good.goodsDesc || "",
      vendingGoodUuid: good.uuid, // Renamed from machineUuid
    ...
    const loadStateFromApi = async () => {
      ...
      try {
        ...
        if (cart && (cart.items?.length > 0 || (cart.current_step && cart.current_step > 1))) {
          ...
          if (restoredStep === 1 && (cart.plan_type !== "NONE" || (cart.items && cart.items.length > 0) || cart.location)) {
            restoredStep = 2;
          }
          ...
          if (rawSubtype && rawSubtype !== "NONE") {
            setPlanType(rawSubtype.toLowerCase());
          }
          if (cart.pickup_type) {
            const pMap: any = {
              TODAY: "Pickup Today",
          ...
          const restoredItems = cart.items.map((item: any) => ({
            ...
            imgAlt: `food-${item.menu_item.id}`, // CRITICAL: Match Menu.tsx format
            imgSrc: item.menu_item.image_url || "/images/placeholder_food.png", // CRITICAL: Match Menu.tsx format
            quantity: item.quantity,
            id: item.menu_item.id,
            price: `AED ${parseFloat(item.menu_item.price).toFixed(2)}`, // Ensure price format matches Menu.tsx
          }));
        ...
        days.forEach((day) => {
          menuData[day]?.forEach((item) => {
            allItems.push({
              ...
              vending_good_uuid: item.vendingGoodUuid || null, // NEW: Include Good UUID
            });
          });
        });
        ...
        <div className="w-full py-6">
          <div className="main-container space-y-4">
            ...
            {step1Status !== "pending" && (
              <div
                key={1}
                className={`w-full border rounded-[16px] transition-all ${step1Status === "active"
                  ? "border-[#EDEEF2] bg-white shadow-lg"
                  : "border-[#EDEEF2] bg-white"
                  }`}>
                <div className="py-[20px] md:px-[24px] px-3">
                  <div className="flex flex-row justify-between max-md:gap-4">
                    <div className="flex gap-4">
                      <div
                        className={`md:h-[32px] h-[26px] w-[26px] md:w-[32px] rounded-full inline-flex items-center justify-center flex-shrink-0 transition-all ${step1Status === "completed"
                          ? "bg-[#10B981] text-white"
                          : "bg-[#054A86] text-white"
                          }`}>
            ...
            {step2Status !== "pending" && (
              <div
                key={2}
                className={`w-full border rounded-[16px] transition-all ${step2Status === "active"
                  ? "border-[#EDEEF2] bg-white shadow-lg"
                  : "border-[#EDEEF2] bg-white"
                  }`}>
                <div className="py-[20px] md:px-[24px] px-3">
                  <div className="flex flex-row justify-between max-md:gap-4">
                    <div className="flex gap-4">
                      <div
                        className={`md:h-[32px] h-[26px] w-[26px] md:w-[32px] rounded-full inline-flex items-center justify-center flex-shrink-0 transition-all ${step2Status === "completed"
                          ? "bg-[#10B981] text-white"
                          : "bg-[#054A86] text-white"
                          }`}>
                                ...
                                <div className="flex gap-4 pb-6">
                                  {planSubTypes.map((opt) => (
                                    <Button
                                      ...
                                      className={` ${planType === opt.key.toLowerCase()
                                        ? "bg-[#EAF5FF] hover:bg-[#EAF5FF] border border-[#054A86] text-[#2B2B43]"
                                        : "bg-neutral-white hover:bg-neutral-white border border-[#C7C8D2] text-[#2B2B43]"
                                        } px-6 py-3 md:rounded-[16px] rounded-[10px] font-bold`}>
                                      {opt.label}
            ...
            {step3Status !== "pending" && (
              <div
                key={3}
                className={`w-full border rounded-[16px] transition-all ${step3Status === "active"
                  ? "border-[#EDEEF2] bg-white shadow-lg"
                  : "border-[#EDEEF2] bg-white"
                  }`}>
                <div className="py-[20px] md:px-[24px] px-3">
                  <div className="flex flex-row justify-between max-md:gap-4">
                    <div className="flex gap-4">
                      <div
                        className={`md:h-[32px] h-[26px] w-[26px] md:w-[32px] rounded-full inline-flex items-center justify-center flex-shrink-0 transition-all ${step3Status === "completed"
                          ? "bg-[#10B981] text-white"
                          : "bg-[#054A86] text-white"
                          }`}>
            ...
            {orderType === "Order Now" && step4Status !== "pending" && (
              <div
                key={4}
                className={`w-full border rounded-[16px] transition-all ${step4Status === "active"
                  ? "border-[#EDEEF2] bg-white shadow-lg"
                  : "border-[#EDEEF2] bg-white"
                  }`}>
                <div className="py-[20px] md:px-[24px] px-3">
                  <div className="flex flex-row justify-between max-md:gap-4">
                    <div className="flex gap-4">
                      <div
                        className={`md:h-[32px] h-[26px] w-[26px] md:w-[32px] rounded-full inline-flex items-center justify-center flex-shrink-0 transition-all ${step4Status === "completed"
                          ? "bg-[#10B981] text-white"
                          : "bg-[#054A86] text-white"
                          }`}>
                  ...
                  {step4Status === "active" && (
                    <div className="mt-4 pl-0 ">
                      <Menu
                        ...
                        machineGoods={machineGoods} // Pass machineGoods (can be null)
                      />
                    </div>
                  )}
                </div>
            ...
            {orderType === "Start a Plan" && planType === "monthly" && (
              <>
                ...
                {step5Status !== "pending" && (
                  <div
                    key={5}
                    className={`w-full border rounded-[16px] transition-all ${step5Status === "active"
                      ? "border-[#EDEEF2] bg-white shadow-lg"
                      : "border-[#EDEEF2] bg-white"
                      }`}>
                    <div className="py-[20px] md:px-[24px] px-3">
                      <div className="flex flex-row justify-between max-md:gap-4">
                        <div className="flex gap-4">
                          <div
                            className={`md:h-[32px] h-[26px] w-[26px] md:w-[32px] rounded-full inline-flex items-center justify-center flex-shrink-0 transition-all ${step5Status === "completed"
                              ? "bg-[#10B981] text-white"
                              : "bg-[#054A86] text-white"
                              }`}>
                ...
                {step6Status !== "pending" && (
                  <div
                    key={6}
                    className={`w-full border rounded-[16px] transition-all ${step6Status === "active"
                      ? "border-[#EDEEF2] bg-white shadow-lg"
                      : "border-[#EDEEF2] bg-white"
                      }`}>
                    <div className="py-[20px] md:px-[24px] px-3">
                      <div className="flex flex-row justify-between max-md:gap-4">
                        <div className="flex gap-4">
                          <div
                            className={`md:h-[32px] h-[26px] w-[26px] md:w-[32px] rounded-full inline-flex items-center justify-center flex-shrink-0 transition-all ${step6Status === "completed"
                              ? "bg-[#10B981] text-white"
                              : "bg-[#054A86] text-white"
                              }`}>
                ...
                {step7Status !== "pending" && (
                  <div
                    key={7}
                    className={`w-full border rounded-[16px] transition-all ${step7Status === "active"
                      ? "border-[#EDEEF2] bg-white shadow-lg"
                      : "border-[#EDEEF2] bg-white"
                      }`}>
                    <div className="py-[20px] md:px-[24px] px-3">
                      <div className="flex flex-row justify-between max-md:gap-4">
                        <div className="flex gap-4">
                          <div
                            className={`md:h-[32px] h-[26px] w-[26px] md:w-[32px] rounded-full inline-flex items-center justify-center flex-shrink-0 transition-all ${step7Status === "completed"
                              ? "bg-[#10B981] text-white"
                              : "bg-[#054A86] text-white"
                              }`}>
