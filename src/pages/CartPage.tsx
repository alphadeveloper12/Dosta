 ...
 useEffect(() => {
  const fetchMenuAndCart = async () => {
   try {
    ...
    const menuRes = await axios.get(`${baseUrl}/api/vending/menu/ORDER_NOW/`, {
     headers: { Authorization: `Token ${token}` },
    });
  ...
  const checkStock = async () => {
    ...
    const updatedItems = items.map((item) => {
     ...
     if (
      ...
      if (item.name.toLowerCase().includes("chicken pesto pasta")) {
       console.log(
        `[Stock Check] Chicken Pesto Pasta (UUID: ${item.vendingGoodUuid}) - Available: ${available}`,
       );
      }
     } else {
      ...
      if (item.name.toLowerCase().includes("chicken pesto pasta")) {
       console.log(
        `[Stock Check] Chicken Pesto Pasta (Name: ${normName}) - Available: ${available}`,
       );
      }
     }
     ...
     if (available !== undefined) {
      ...
      if (sourceName === "API") {
       // Only warn on API source
       console.warn(
        `[${sourceName}] Stock Check: Item "${item.name}" not found in machine data.`,
       );
      }
     }
     return item;
     ...
     updatedItems.forEach(async (item) => {
      ...
      if (original && original.quantity !== item.quantity) {
       try {
        ...
        console.log(
         `[${sourceName}] ✅ Synced adjusted quantity for ${item.name} to ${item.quantity}`,
        );
       } catch (e) {
        console.error(
         `[${sourceName}] Failed to sync adjustment for ${item.name}`,
         e,
        );
       }
      }
   ...
   try {
    ...
    if (loadedCount === 0 && goodsData.data) {
     ...
     console.log(
      `📦 API Fallback: Loaded ${allGoods.length} goods from legacy list.`,
     );
    ...
    if (freshStockMap["1064051"]) {
     console.log(
      `[API] Debug Stock: Beef And Broccoli (UUID: 1064051) = ${freshStockMap["1064051"]}`,
     );
    }
        ...
        onCheckout={async () => {
         ...
         try {
          ...
          if (!Array.isArray(paymentRes.data) || paymentRes.data.length === 0) {
           toast.error(
            "No payment information found. Please add a payment method in your profile.",
           );
          ...
          toast.error(
           "Unable to verify payment information. Please try again.",
          );
          return;
         }
