...
const ChatBot = () => {
    ...
    const [messages, setMessages] = useState<Message[]>([
        { role: "bot", content: "Hello! I'm Dosta's AI assistant. How can I help you today?" },
    ]);
                        ...
                        <div className="flex-1 overflow-y-auto p-4 space-y-4 bg-neutral-gray-lightest/30">
                            {messages.map((msg, idx) => (
                                ...
                                >
                                    ...
                                    >
                                        <div
                                            className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 ${msg.role === "user" ? "bg-primary-light" : "bg-[#054A86] text-white"
                                                }`}
                                        >
                                            {msg.role === "user" ? <User size={16} /> : <Bot size={16} />}
                                        </div>
                                        <div
                                            className={`p-3 rounded-[16px] text-[14px] leading-[1.4] shadow-sm ${msg.role === "user"
                            ...
                            {isLoading && (
                                <div className="flex justify-start">
                                    <div className="flex gap-2 items-center bg-white border border-neutral-gray-light p-3 rounded-[16px] rounded-tl-none shadow-sm">
                                        <Loader2 className="animate-spin text-[#054A86]" size={16} />
                                        <span className="text-[14px] text-neutral-gray italic">Thinking...</span>
                                    </div>
                                </div>
                            )}
