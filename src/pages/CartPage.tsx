 ...
 return (
  <div className="bg-gray-50 min-h-screen max-md:pb-[82px]">
   ...
   <MobileFooterNav />
    ...
    const [loading, setLoading] = useState<boolean>(true);
    const [isCheckingOut, setIsCheckingOut] = useState<boolean>(false); // NEW: Checkout loading state
    const navigate = useNavigate();
    const baseUrl = import.meta.env.VITE_API_URL;
                            ...
                            <OrderSummary
                                ...
                                onCheckout={async () => {
                                    ...
                                    try {
                                        ...
                                        if (serialNumber) {
                                            console.log("🔍 Validating items with Vending Machine:", serialNumber);
                                            ...
                                            if (goodsData.data) {
                                                ...
                                                updatedItems = items.map(cartItem => {
                                                    const normalizedCartName = normalizeName(cartItem.name);
                                                    const match = allGoods.find((good: any) =>
                                                        normalizeName(good.goodsName) === normalizedCartName
                                                    );
                                                    if (match) {
                                                        console.log(`✨ Matched "${cartItem.name}" -> ${match.uuid}`);
                                        ...
                                        const orderRes = await axios.post(`${baseUrl}/api/vending/order/confirm/`, payload, {
                                            headers: { Authorization: `Token ${token}` },
                                        });
                                            ...
                                            if (matchedGoods.length > 0) {
                                                ...
                                                if (pickData.result === "200" && pickData.data) {
                                                    ...
                                                    matchedGoods.forEach(mg => {
                                                        ...
                                                        // Fallback key using menuItemId
                                                        const item = updatedItems.find(i => i.vendingGoodUuid === mg.goodsUuid);
                                                        if (item) {
                                                            existingCodes[`menu_${item.menuItemId}`] = pickData.data;
                                                        }
                                                    });

                                                    localStorage.setItem(storageKey, JSON.stringify(existingCodes));
                                                }
                                            }
                                        }
                                        ...
                                        // --- 4. Clear Cart ---
                                        try {
                                            await axios.post(`${baseUrl}/api/vending/cart/`, { items: [], current_step: 1 }, {
                                                headers: { Authorization: `Token ${token}` },
                                            });
                                            setItems([]);
