...
const MainSection: React.FC = () => {
 ...
 const googleLogin = useGoogleLogin({
  onSuccess: async (tokenResponse) => {
   try {
    ...
    // ✅ store auth in both for consistency and persistence
    ...
    if (guestCartData) {
     try {
      ...
      console.error("Direct Google Login Cart Sync Error:", syncErr);
     }
    }
 ...
 const AuthPanel: React.FC = () => {
  ...
  const handleContinue = () => {
   // Prevent actions if no method selected (regardless of device if we want to be strict,
   // but primarily for Mobile flow).
   // If on Desktop, "Continue" button is effectively disabled/useless if we remove the dashboard redirect.
    ...
    <div className="max-w-sm w-full mx-auto flex flex-col h-full md:justify-start justify-between pb-[32px]">
     <div>
      ...
      <p className="mt-4 text-base font-[400] text-[#545563]">
       ...
       and{" "}
       <Link
        className="underline text-[#056AC1] font-bold"
        to="/privacy-policy">
        Privacy Policy
       </Link>
       .
      ...
      <div className="pt-8 space-y-5">
       ...
       <Button
        ...
        className={`w-full justify-start gap-3 max-md:px-[16px] md:py-5 ${
         isMobile && selectedMethod === "email"
          ? "border border-blue-400"
          : "border border-neutral-gray-lightest"
        }`}
        onClick={() =>
         isMobile ? setSelectedMethod("email") : setShowEmailForm(true)
        }>
        <EmailIcon />
       ...
       <Button
        ...
        size="xlg"
        className={`w-full justify-start gap-3 max-md:px-[16px] md:py-5 ${
         isMobile && selectedMethod === "google"
          ? "border border-blue-400"
          : "border border-neutral-gray-lightest"
        }`}
        onClick={() =>
      ...
      <Button
       ...
       disabled={isMobile && !selectedMethod}>
       Continue
      </Button>
     </div>
