  ...
  const handleConfirmAndPay = async () => {
    try {
      ...
      const orderData = {
        ...
        // Extra details for confirmation page recovery
        extraDetails: {
          cuisines: selectedCuisines,
          courses: selectedCourses,
          budget: selectedBudget,
          menuItems: selectedMenuItems,
          pricing: { baseTotal, vat, total },
        },
      };

      // 1. Save pending order to localStorage
      localStorage.setItem("pendingCateringOrder", JSON.stringify(orderData));

      // 2. Initiate Payment Session (Generic Call to Vending API)
      const description = `Catering: ${orderData.event_type} (${guestCount} guests)`;
      const returnUrl = `${window.location.origin}/catering/plan?payment_success=true`;

      console.log("💳 Initiating Payment Session for Catering...");
      const payRes = await fetch(`${baseUrl}/api/vending/payment/initiate/`, {
        method: "POST",
        headers: {
        ...
        body: JSON.stringify({
          ...
          cancel_url: `${window.location.origin}/catering/plan?payment_cancelled=true`,
        }),
      });

      if (payRes.ok) {
        const payData = await payRes.json();
        if (payData.payment_redirect_url) {
          console.log("✈️ Redirecting to Payment Gateway...");
          window.location.href = payData.payment_redirect_url;
        } else {
      ...
      console.error("Error during catering checkout:", error);
          ...
          <div className="flex flex-col lg:flex-row gap-6">
              ...
              <div
                ...
                <div className="mb-6 pb-6">
                  ...
                  {selectedServiceStyles?.name?.toLowerCase().includes("iftar") &&
                    ...
                    <p className="text-blue-600 font-medium">Iftar Boxes Menu</p>
            ...
            <div className="lg:w-1/3">
              <div className="bg-white rounded-lg p-6 shadow-sm border border-gray-200 sticky top-8">
                ...
                {!sessionStorage.getItem("authToken") && !localStorage.getItem("authToken") && (
                  <div className="bg-[#F0F7FF] border border-[#056AC1]/20 p-4 rounded-xl mb-6">
                    <p className="text-[13px] text-[#056AC1] font-[500] leading-relaxed">
                      You are booking as a <strong>Guest</strong>.
                      Sign in to save this booking to your account and manage it easily later.
                    </p>
                    <button
                      onClick={() => setShowAuthModal(true)}
                      className="text-[13px] text-[#054A86] font-[700] underline mt-1 hover:text-[#0768B8] transition-colors"
                    >
                      Sign In Now
                    </button>
                  </div>
