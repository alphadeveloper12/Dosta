   ...
   foodData.filter(matchesCategory).forEach((item) => {
    ...
    const isAvailable = (machineGoods || []).some((good) => {
     ...
     if (normalizedGoodName === normalizedItemName) {
      ...
      if (
       typeof good === "object" &&
       good.presentNumber !== undefined &&
       good.presentNumber <= 0
      ) {
       return false;
      }
      return true;
    ...
    data.menus?.forEach((menu: any) => {
     menu.items?.forEach((it: any) => {
      allItems.push({
       ...
       imgSrc2: it.image2_url || imageMap[it.name] || it.image_url || "/images/placeholder_food.png",
       heading: it.name,
       imgAlt: `food-${it.id}`,
  ...
  if (shelfData && shelfData.length > 0) {
   shelfData.forEach((shelf: any) => {
    shelf.spots.forEach((spot: any) => {
     if (
      spot.enrichedItem &&
      normalizeName(spot.enrichedItem.heading) === normalizeName(item.heading)
     ) {
     ...
     <div className="flex flex-row justify-between items-center gap-4">
      <div className="flex-1">
       ...
       <p className="text-[#545563] text-[14px] leading-[20px] font-[400] tracking-[0.1px]">
        Choose your meal from our daily menu of {totalAvailableCount}{" "}
        chef-prepared meals
       </p>
       {machineGoods === null && (
        <div className="flex items-center gap-2 mt-1">
         <img src="/animated-icon.svg" alt="Loading..." className="w-4 h-4" />
        ...
        <div className="space-y-12">
         {shelfData.map((shelf: any) => (
          <div
           ...
           className="bg-gray-50/50 rounded-[24px] p-4 md:p-8 border border-gray-100">
             ...
             .map((spot: any, index: number) => {
              ...
              return (
               <div key={index} className="relative">
                ...
                <div
                 className={
                  isSoldOut || isLocked ? "opacity-60 grayscale-[0.5]" : ""
                 }>
                 <MenuCard
                  data={data}
                  itemInCart={itemInCart}
                  handleCardClick={(item) => !isLocked && handleCardClick(item)}
                  handleQuantityChange={(e, item, delta) =>
                   !isLocked && handleQuantityChange(e, item, delta)
                  }
                 />
                </div>
                {isLocked ? (
