...
const Arrow: React.FC<{ dir: "left" | "right"; onClick: () => void }> = ({
  ...
  <button
    ...
    className={`absolute top-[65%] hidden md:block -translate-y-1/2 z-20 rounded-full p-2 border border-[#2B2B43] ${dir === "left"
...
const Dot: React.FC<{ selected: boolean; onClick: () => void }> = ({
  ...
  <button
    type="button"
    onClick={onClick}
    className={`h-2.5 rounded-full transition-all ${selected ? "bg-black/80 w-6" : "bg-black/30 hover:bg-black/50 w-2.5"
      }`}
