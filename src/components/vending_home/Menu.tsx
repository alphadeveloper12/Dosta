...
const Menu: React.FC<MenuProps> = ({
    ...
    const { availableItems, otherItems } = React.useMemo(() => {
        ...
        console.log("📦 Vending Machine Menu (Machine Goods):", JSON.stringify(machineGoods, null, 2));
        console.log("🍱 Order Now Menu (Full Food Data):", JSON.stringify(foodData, null, 2));
        ...
        foodData.forEach(item => {
            ...
            const isAvailable = machineGoods.some(good => {
                ...
                if (normalizedGoodName === normalizedItemName) {
                    ...
                    console.log(`✅ Match Found: "${item.heading}" (normalized: "${normalizedItemName}") matched with machine good "${rawName}" (normalized: "${normalizedGoodName}") with Good UUID: ${matchedUuid}`);
                    return true;
                }
                return false;
            });
        ...
        console.log(`📊 Full List Results -> Available: ${available.length}, Others: ${others.length}`);

        // --- NEW: Deduplicate while prioritizing available items ---
        // 1. Deduplicate available items
        const uniqueAvailable = Array.from(new Map(available.map(item => [item.heading, item])).values());
        ...
        console.log(`📊 Final Deduplicated Results -> Available: ${uniqueAvailable.length}, Others: ${uniqueOthers.length}`);
...
>>>>>>> Stashed changes
                ...
                // --- UPDATED: Store all items, deduplication will happen in useMemo ---
                console.log("📋 All Menu Items (Before Deduplication):", JSON.stringify(allItems, null, 2));
    ...
    const handleQuantityChange = (
        ...
        setCart((prevCart) => {
            ...
            console.log("🛒 Updated Cart with Good UUIDs:", JSON.stringify(newCart, null, 2));
            return newCart;
        });
    };
        ...
        <div className="w-full">
            <main className="md:pt-0 pt-6">
                ...
                <div className="sticky top-16 z-30 bg-white/90 backdrop-blur-md py-4 mb-8 hidden md:block border border-gray-100 shadow-md rounded-[20px] px-8 transition-all duration-300">
                    <div className="flex flex-row justify-between items-center gap-4">
                        <div className="flex-1">
                            <h2 className="text-[24px] font-bold text-[#054A86] mb-1">Choose Your Meal</h2>
                            ...
                            {machineGoods === null && (
                                <div className="flex items-center gap-2 mt-1">
                                    ...
                                    <span className="text-[12px] text-[#054A86] font-medium">Checking availability...</span>
                                </div>
                            )}
                        </div>
                        <div className="flex items-center gap-8">
                            <div className="text-right">
                                ...
                                <p className="text-[22px] font-bold text-[#054A86]">Total: {totalMeals} Meals</p>
                            </div>
                ...
                <div className="md:hidden mb-6 px-1">
                    <h2 className="text-[22px] font-bold text-[#054A86] mb-1">Choose Your Meal</h2>
                    ...
                    {!loading && !error && (
                        <>
                            ...
                            {availableItems.length > 0 && (
                                <div className="mb-8">
                                    <h3 className="text-[20px] font-bold mb-4 text-[#2B2B43] px-1">Available Menu</h3>
                                    <div className="grid grid-cols-12 md:flex md:gap-[24px] gap-[12px] flex-wrap">
                            ...
                            {otherItems.length > 0 && (
                                <div className="mb-8">
                                    {machineGoods !== null && (
                                        <h3 className="text-[20px] font-bold mb-4 text-[#2B2B43] px-1">Others</h3>
                                    )}
                                    <div className="grid grid-cols-12 md:flex md:gap-[24px] gap-[12px] flex-wrap">
