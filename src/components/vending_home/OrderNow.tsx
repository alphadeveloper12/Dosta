...
const generateCartSummary = (cartData: any[]) => {
  ...
  const line = cart
    .map(
      (item) =>
        `${item.heading}${item.quantity > 1 ? ` (x${item.quantity})` : ""}`,
    )
    .join(", ");
...
const buildConfirmPayload = ({
  ...
  const formatItems = (data: any, pType: string) => {
    ...
    const buildMenuItem = (it: any) => ({
      id: it.id,
      name: it.heading || it.name,
      price: (it.price || "0").toString().replace("AED ", ""),
      image_url: it.imgSrc || it.image_url || null,
      heating: it.heating || "yes",
    ...
    if (pType === "weekly") {
      ...
      days.forEach((day) => {
        (data[day] || []).forEach((item: any, idx: number) => {
          const tempId = Math.floor(Math.random() * 1000000);
          items.push({
            id: tempId,
            menu_item_id: item.id,
            menu_item: buildMenuItem(item),
...
const OrderNow = () => {
    ...
    const loadStateFromApi = async () => {
      ...
      try {
        ...
        if (cartData) {
          ...
          // Check if local selection differs from cart location
          let useCart = true;
          try {
            ...
            if (localData?.location?.id && cart?.location?.id) {
              if (String(localData.location.id) !== String(cart.location.id)) {
                // User selected a different location locally; ignore stale cart
                useCart = false;
              }
            }
          } catch (e) {
  ...
  // load pickup location from localStorage/sessionStorage when it changes
  useEffect(() => {
    try {
      const selectedLocation = JSON.parse(
        localStorage.getItem("selectedLocation") ||
  ...
  const smartGrabAvailableItems = useMemo(() => {
    ...
    return machineGoods
      .filter((good: any) => {
        // We now allow all items through so they can show as LOCKED or SOLD OUT
        return true;
      })
      .map((good: any) => ({
        id: good.uuid,
  ...
  const confirmSavePlan = () => {
    ...
    if (isDefaultPlan) {
      updated = updated.map((p) =>
        p.id === newPlan.id ? p : { ...p, isDefault: false },
      );
    }
  ...
  const handleConfirmStep = async () => {
    ...
    const payload: any = {
      ...
      plan_subtype:
        orderType === "Start a Plan"
          ...
          : "NONE",
      pickup_type:
        pickOrder === "Pickup in 24 hours" || pickOrder === "Pickup in 24"
          ? "IN_24_HOURS"
          : "TODAY",
      pickup_date: new Date().toISOString().split("T")[0],
      pickup_slot_id:
    ...
    try {
      if (token) {
        ...
        // Update Redux state for local persistence
        // @ts-ignore
                  ...
                  <div className="flex flex-row justify-between max-md:gap-4">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        ...
                        {(step1Status === "completed" || (step1Status === "active" && pickupLocation !== "no location selected")) && (
                    ...
                    {step1Status === "completed" ? (
                      ...
                      <Button
                        ...
                        className="bg-[#054A86] text-white md:text-[14px] py-[6px]! px-4! text-[12px] leading-[18px] font-[700] tracking-[0.3px] rounded-[8px]">
                      ...
                      <div className="flex md:flex-row flex-col items-start gap-8">
                        <div className="flex-1">
                          ...
                          <p className="text-[14px] leading-[20px] text-[#545563] mb-6">
                            ...
                            to Friday. Simply place your order now, and we&apos;ll make it
                            fresh just for you. Your meal will be stocked in our vending
                      ...
                      {orderType !== "Start a Plan" && (
                        <div className="flex gap-4 flex-row">
                          {pickupOptions
                            .filter(
                              (opt: any) =>
                                !(orderType === "Order Now" && opt.label === "Pickup Today"),
                            )
                            .map((opt: any) => (
                              <Button
                                key={opt.key}
